...
Pins the JSON shape of the optional FastAPI surface. The CLI is the primary
interface, but the API remains available for programmatic use, so these
assertions guard its response contract.
"""
...
def test_health_exposes_credentials_configured_for_the_ui(monkeypatch):
    """Clients read credentials_configured to decide whether to send keys."""
...
# ---------------------------------------------------------- /query contract
def test_query_response_contains_every_contract_field(stub_pipeline):
    stub_pipeline()
    with TestClient(backend.main.app) as client:
        r = client.post(
            "/query",
    ...
    for field in (
        ...
        assert field in body, f"contract field '{field}' missing from the response"

    assert isinstance(body["sources"], list)
    assert isinstance(body["latency"], (int, float))   # seconds
...
def test_all_retrieval_status_values_are_reachable(stub_pipeline, status):
    """Every retrieval_status value a client may render must be reachable."""
...
def test_client_can_omit_keys_when_server_has_them(monkeypatch, stub_pipeline):
    """A client may send no key fields; the server uses its own."""
    monkeypatch.setenv("AWS_BEARER_TOKEN_BEDROCK", "server-llm")
    monkeypatch.setenv("SERPER_API_KEY", "server-search")
    captured: dict = {}
...
def test_errors_return_a_string_detail(stub_pipeline):
    """Clients read payload.detail as a string on non-2xx."""
...
def test_browser_origin_is_allowed_when_configured(monkeypatch):
    ...
    monkeypatch.setenv("AUTOSEARCH_ALLOWED_ORIGINS", CLIENT_ORIGIN)
    ...
    try:
        with TestClient(module.app) as client:
            ...
            health = client.get("/health", headers={"Origin": CLIENT_ORIGIN})

        assert preflight.status_code in (200, 204)
        assert preflight.headers["access-control-allow-origin"] == CLIENT_ORIGIN
        assert health.headers["access-control-allow-origin"] == CLIENT_ORIGIN
    finally:
